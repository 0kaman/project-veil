/**
 * A hardened WebSocket JSON-RPC client for the Chrome DevTools Protocol.
 *
 * CDP is just {id, method, params} over a socket, plus pushed events. The whole
 * value of this file is that it never lets the socket take down the process:
 * a malformed frame is dropped, a send after close fails fast, and a throwing
 * event handler can't abort delivery to the others. Carried from v1, where each
 * of those was a real crash before it was a comment.
 */
...
export function createCDPClient(wsUrl: string): Promise<CDPClient> {
  return new Promise((resolve, reject) => {
    ...
    ws.addEventListener("message", (event) => {
      ...
      // handler runs on an EventTarget with no surrounding try/catch — a raw
      // JSON.parse throw here once killed a whole daemon and every session it held.
      let msg: {
        id?: number;
        method?: string;
      ...
      if (typeof msg.id === "number") {
        ...
        if (req) {
          ...
          if (msg.error) {
            req.reject(new Error(`CDP error: ${msg.error.message} (${msg.error.code})`));
          } else {
    ...
    ws.addEventListener("error", (e) => {
      if (!closed) debugLog("cdp: websocket error", String((e as ErrorEvent)?.message ?? e));
